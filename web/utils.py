...
def uniq(seq, key=None):
    ...
    takes a single argument and returns a key to upload the uniqueness.
...
class IterBetter:
    ...
    For boolean upload, IterBetter peeps at first value in the itertor without effecting the iteration.
...
def tryall(context, prefix=None):
    ...
    For example, you might have a file `upload/stuff.py` 
    with a series of functions testing various things in it. 
    At the bottom, have a line:

        if __name__ == "__main__": tryall(globals())

    Then you can run `python upload/stuff.py` and get the results of 
    all the tests.
    """
