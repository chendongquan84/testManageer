...
class SQLParam(object):
    ...
    Parameter in SQLQuery.
        ...
        >>> q = SQLQuery(["SELECT * FROM upload WHERE name=", SQLParam("joe")])
        >>> q
        <sql: "SELECT * FROM upload WHERE name='joe'">
        >>> q.query()
        'SELECT * FROM upload WHERE name=%s'
        >>> q.values()
        ['joe']
    """
    __slots__ = ["value"]
...
class SQLQuery(object):
    ...
    def __init__(self, items=None):
        r"""Creates a new SQLQuery.
            ...
            >>> SQLQuery("x")
            <sql: 'x'>
            >>> q = SQLQuery(['SELECT * FROM ', 'upload', ' WHERE x=', SQLParam(1)])
            >>> q
            <sql: 'SELECT * FROM upload WHERE x=1'>
            >>> q.query(), q.values()
            ('SELECT * FROM upload WHERE x=%s', [1])
            >>> SQLQuery(SQLParam(1))
            <sql: '1'>
        """
        if items is None:
...
class MSSQLDB(DB):
    ...
    def _test(self):
        """Test LIMIT.
            ...
            >>> db = MSSQLDB(db='upload', user='joe', pw='secret')
...
def register_database(name, clazz):
    ...
    Register a database.
        ...
        >>> db = database(dbn='legacy', db='upload', user='joe', passwd='secret') 
    """
